...
2. `generate_screen_from_text` with:
   ...
   - `modelId`: GEMINI_3_PRO (foundation screen is critical — use pro model)
...
8. Present screenshot to user via `Read` (after download is confirmed):
   ...
   - If user wants changes: `edit_screens` with user feedback → re-run **Asset Download Procedure** → re-run **Design Token Extraction** → update DESIGN.md
