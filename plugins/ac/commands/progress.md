...
You are reading `.ac/plans/` and `.ac/tasks/` to display a structured execution status dashboard. Read files, parse state, and present a concise report. No agents needed — read and present only.
...
5. If all three lists are empty after filtering → print "No active plans or tasks. Start with `/ac:ideate` or `/ac:plan`." and stop.
...
7. If no wisdom file → Status = "Pending execution".
