...
This command spawns agents to do the work — it does NOT implement code itself.
...
4. Parse the plan into structured steps:
   ...
   - Tier field (quick/mid/senior, if specified)
5. **Expected Plan Format**: Plans generated by ac:plan follow this structure:
   - `# Plan: [Title]` heading
   - Steps with `**Step N**:`, `Files:`, `Done when:`, `Independence:`, `Tier:` fields
   - `### Waves` section with parallel decomposition (or legacy `### Work Units`)
   - `### Must NOT Have` section with exclusions

   If neither Waves nor Work Units section is found, auto-analyze step independence based on file overlap (existing behavior). If plan format doesn't match expected structure, warn user and attempt best-effort parsing.
...
7. **Extract tier assignments**: For each parsed step, read the `Tier:` field. Map to model: `quick`→`haiku`, `mid`→`sonnet`, `senior`→`opus`. If `Tier:` is absent, check for legacy `Escalate:` field as fallback: `true`→senior (Opus), `false` or absent→mid (Sonnet). This ensures backward compatibility with plans generated before the tier system.
