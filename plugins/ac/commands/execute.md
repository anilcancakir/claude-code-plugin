...
4. Parse the plan into structured steps:
   ...
   - Escalate flag (true/false, if specified)
5. **Expected Plan Format**: Plans generated by ac:plan follow this structure:
   - `# Plan: [Title]` heading
   - Steps with `**Step N**:`, `Files:`, `Done when:`, `Independence:`, `Escalate:` fields
...
Agent(
  ...
  model: "[omit for Sonnet default, or \"opus\" if step has Escalate: true]",
  run_in_background: true
)
```

**Model routing**: If the step (or any step in the Work Unit) has `Escalate: true`, add `model: "opus"` to the Agent() call. If `Escalate: false` or absent, omit the `model` parameter — the agent inherits Sonnet by default. For Work Units containing mixed-escalation steps, use the highest tier: if ANY constituent step has `Escalate: true`, the entire unit's agent gets `model: "opus"`.
