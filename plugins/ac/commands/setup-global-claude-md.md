...
3. Detect marketplace plugin skills from session's available skills list. Namespaced `<plugin>:<skill>` format. Exclude `skill-creator` and `ac:skill-creator`.
...
3. **Enhance/New mode**: Build all sections from interview + detection:
   ...
   - **Skills table**: Merge user skills (`~/.claude/skills/`) + marketplace plugin skills (namespaced). Exclude skill-creator
