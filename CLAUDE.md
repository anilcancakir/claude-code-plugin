...
- **Foreground-first agent synchronization**: Parallel agents that must all complete before proceeding use foreground mode (CC's `Promise.all` — automatic barrier). Background (`run_in_background: true`) only for genuinely independent work with explicit wave completion barrier (count notifications vs launched agents). Verification agents are always foreground. All parallel Agent calls must be in a single assistant message block.
